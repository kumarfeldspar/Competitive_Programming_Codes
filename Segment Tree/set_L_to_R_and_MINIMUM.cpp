...
public:
    SegmentTree(int size) : n(size), tree(4 * size, 0), lazy(4 * size, 0), lazyFlag(4 * size, false) {}
    ...
    void propagate(int node, int start, int end)
    {
        if (lazyFlag[node])
        {
            ...
            if (start != end)
            {
                ...
                lazyFlag[node * 2 + 1] = true;
                lazyFlag[node * 2 + 2] = true;
            }
            lazyFlag[node] = false;
        }
    }

    void update_range(int l, int r, long long value, int start, int end, int node)
    {
